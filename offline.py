...
from utils import get_layout_from_file

root = 'E:/inria-bci-challenge/train/'
channel_loc_fp = 'E:/inria-bci-challenge/ChannelsLocation.csv'
train_label_location = 'E:/inria-bci-challenge/TrainLabels.csv'
files = [os.path.join(root, fn) for fn in os.listdir(root)]

eeg_channels, layout = get_layout_from_file(channel_loc_fp)
n_channels = 59
n_eeg_channels = 56
...
array_eeg_copy[:, -1][np.argwhere(array_eeg_copy[:, -1])] = array_prediction[:np.count_nonzero(np.argwhere(array_eeg_copy[:, -1]))]

mne_raw = mne.io.RawArray(array_eeg_copy.transpose(), mne.create_info(['Time'] + eeg_channels + ['eog', 'stim'], 200, ch_types=['misc'] + ['eeg'] * n_eeg_channels + ['eog', 'stim']))
mne_raw.set_montage(mne.channels.make_standard_montage('standard_1020'), on_missing='warn')
# fig = mne_raw.plot_psd(average=True, show=False)
# plt.title('Before Re-referencing')
# plt.show()
mne_raw, _ = mne.set_eeg_reference(mne_raw, 'average', projection=False)
# fig = mne_raw.plot_psd(average=True, show=False)
# plt.title('After Re-referencing')
# plt.show()
